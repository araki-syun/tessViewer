...
namespace tv {
	...
	glslStringDraw * glslStringDraw::getInstance()
	{
		return &stringDraw;
	}
	...
	void glslStringDraw::Initialize(int fontsize, const std::string & fontname)
	{
		...
		glTextureImage2DEXT(temp_texture, GL_TEXTURE_2D, 0, GL_RED, tex_size, tex_size, 0, GL_RED, GL_UNSIGNED_BYTE, nullptr);
		...
		for (int i = ' '; i < '~'; ++i) {
			...
			if (FT_Load_Char(ftface, i, FT_LOAD_DEFAULT))
				throw std::runtime_error((boost::format("Load Char : %1%") % (char)i).str());
			if (FT_Render_Glyph(ftface->glyph, FT_Render_Mode::FT_RENDER_MODE_NORMAL))
				throw std::runtime_error("Render Gliph");
			FT_Bitmap& bm = ftface->glyph->bitmap;

			glm::ivec2 pos(
				(i & 0x0F) * (fonttex_size / font_size),
				((i & 0xF0) >> 4) * (fonttex_size / font_size));

			glTextureSubImage2D(
				temp_texture,
				0,
				pos.x + ftface->glyph->metrics.horiBearingX / 64,
				pos.y + font_size - ftface->glyph->metrics.horiBearingY / 64,//- bm->top_margin,
				bm.width,
				bm.rows,
				GL_RED,
				GL_UNSIGNED_BYTE,
			...
			list[i - ' '].wh = (fpos + glm::vec2(font_size, -font_size)) / (float)fonttex_size;
		}
		glFlush();

		std::vector<unsigned char> buf;
		buf.resize(tex_size * tex_size);
		glGetTextureImage(temp_texture, 0, GL_RED, GL_UNSIGNED_BYTE, (int)buf.size(), buf.data());
		...
		glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
		glTextureImage2DEXT(texture, GL_TEXTURE_2D, 0, GL_COMPRESSED_RED, tex_size, tex_size, 0, GL_RED, GL_UNSIGNED_BYTE, flip_img.data);
	...
	{
		glm::vec2 screen_trans((1.f / (float)window_size.x) * 2.f, (1.f / (float)window_size.y) * 2.f);
		glm::vec2 screen_coord((float)x * screen_trans.x - 1.f, float(window_size.y - y) * screen_trans.y - 1.f);
		float offset(0);
		for (int i = 0; i < str.length(); ++i) {
			...
			buffer.emplace_back(screen_coord.x + offset,								screen_coord.y - font_size * screen_trans.y);
			buffer.emplace_back(	uv.xy.x,	uv.wh.y);
			buffer.emplace_back(screen_coord.x + offset + font_size * screen_trans.x,	screen_coord.y - font_size * screen_trans.y);
			buffer.emplace_back(	uv.wh);
			buffer.emplace_back(screen_coord.x + offset + font_size * screen_trans.x,	screen_coord.y);
	...
	void glslStringDraw::Draw()
	{
		...
		glNamedBufferData(ebo, element.size() * sizeof(unsigned int), element.data(), GL_STREAM_DRAW);
		glNamedBufferData(vbo, buffer.size() * sizeof(glm::vec2), buffer.data(), GL_STREAM_DRAW);
		glVertexArrayVertexBuffer(vao, 1, vbo, 0, sizeof(glm::vec2) * 2);
		glVertexArrayAttribFormat(vao, pos, 2, GL_FLOAT, GL_FALSE, 0);
		glVertexArrayAttribFormat(vao, uv, 2, GL_FLOAT, GL_FALSE, sizeof(glm::vec2));
		...
		glProgramUniform3f(prog, program.GetUniform("font_color")->GetIndex(), 1.f, 1.f, 1.f);
