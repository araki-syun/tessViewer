...
int mesh::GetNumFaceIndex(void) const { return _faceIndices.size(); }
...
int mesh::GetNumEdgeIndex(void) const { return _edgeIndices.size(); }
...
void mesh::Refine() {
	...
	_osdMesh.reset(new Osd::Mesh<Osd::CpuGLVertexBuffer, Far::StencilTable,
								 Osd::CpuEvaluator, Osd::GLPatchTable>(
		refiner, numVertexElements, 0, config.Get<int>("OSD.PatchLevel"),
		bits));

	_osdMesh->UpdateVertexBuffer(_vertices.data(), 0,
								 _vertices.size() / numVertexElements);
	_osdMesh->Refine();
}

void mesh::Bind() {
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER,
				 _osdMesh->GetPatchTable()->GetPatchIndexBuffer());
	...
	glVertexAttribPointer(0, numVertexElements, GL_FLOAT, GL_FALSE,
						  numVertexElements * sizeof(float), nullptr);

	glActiveTexture(GL_TEXTURE0);
	glBindBuffer(GL_TEXTURE_BUFFER,
				 _osdMesh->GetPatchTable()->GetPatchParamTextureBuffer());
}
