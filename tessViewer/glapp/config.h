...
template <class... ARGS>
nlohmann::json& Config::_key_value(const ARGS&... keys) {
	...
	for (auto& key : {keys...}) { // schemaからkeyをたどる
		if (auto it_prop = sch.find("properties"); it_prop != sch.cend()) {
			if (auto it_pkey = it_prop->find(key); it_pkey != it_prop->cend()) {
	...
	for (auto& key : {keys...}) { // json設定ファイルからkeyをたどる
		if (auto it = j.find(key); it != j.cend()) { // it が有効なら j に入れる
			j = *it;
	...
	// schが配列型の場合
		...
		if (j.type() == nlohmann::detail::value_t::array) {
			...
			if (auto minItems = sch.find("minItems"); minItems != sch.cend()) {
