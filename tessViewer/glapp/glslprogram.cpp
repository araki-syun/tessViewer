...
namespace glapp{
	const GLuint glslprogram::GetProgram() const{
		return _program;
	}
	...
	void glslprogram::SetShaderName(std::initializer_list<std::string> list){
		...
		for(const std::string& str : list){
			...
			case GL_VERTEX_SHADER:
				shader_list[shader_type::Vertex] = str;
				break;
	...
	void glslprogram::setIndexAttrib(){
		...
		for(int i = 0; i < num_attrib; ++i){
			...
			GLsizei len;
			GLenum type;
			glGetActiveAttrib(_program, i, max_len, &len, &size, &type, name.data());
	...
	void glslprogram::setIndexUniform(){
		...
		for(int i = 0; i < num_uniform; ++i){
			...
			GLsizei len;
			GLenum type;
			glGetActiveUniform(_program, i, max_len, &len, &size, &type, name.data());
