...
namespace glapp{
		...
		_id(0)
	{
		...
		if(!boost::filesystem::exists(file_path))
			throw std::runtime_error((boost::format("NOT FOUND Shader File : %1%") % file_path).str());

		std::ifstream ifs(filename);
		if(ifs.fail())
			throw std::runtime_error((boost::format("LOAD ERROR Shader File : %1%") % file_path).str());
		...
		if(ext == ".vert"){
			...
			throw std::runtime_error((boost::format("") % file_path.string()).str());
		}
	...
	const GLuint glslshader::GetShader() const{
		return _id;
	}

	const std::string& glslshader::GetSource() const{
		return _src;
	}

	const GLuint glslshader::GetType() const{
		return _type;
	}
	...
	const std::string& glslshader::GetError() const{
		return err;
	}
#endif
}
