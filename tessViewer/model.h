...
namespace tv {
	...
	public:
		...
		model(picojson::object& obj, /*std::shared_ptr<glslProgram> p,*/ std::shared_ptr<tv::material> mat);
		~model();

		void Reload( picojson::object& obj);
		void Update(const float* v1, int start1, int size1, const float* v2 = nullptr, int start2 = 0, int size2 = 0);
		void Draw();
	...
	private:
		...
		static OpenSubdiv::Osd::EvaluatorCacheT<OpenSubdiv::Osd::GLComputeEvaluator> evaluator;
	};
	//struct FVarIndex
	//{
	//	int id;
