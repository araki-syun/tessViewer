...
namespace tv {
	glShaderVariable::glShaderVariable():
		_variable_location(-1)
	{
	}
	glShaderVariable::~glShaderVariable()
	{
	}
	const GLuint glShaderVariable::GetIndex() const
	{
	...
	glShaderUniformBlock::glShaderUniformBlock(GLuint program, /*int buf_size,*/ const char * name)/*:
		_buf_size(buf_size)*/
	{
		this->Set(program, name);
	}
	...
	GLuint glShaderUniformBuffer::GetBuffer() const
	{
		return _buffer;
	}
	GLuint glShaderUniformBuffer::GetBinding() const
	{
		return _binding;
	}
