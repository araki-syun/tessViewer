...
app::app(boost::program_options::variables_map& vm)
{
	...
	setting.graphics_osd_patch_level_default = vm[GLAPP_CONFIG_PATCH_LEVEL_DEFAULT].as<int>();
	setting.graphics_osd_patch_level_max = vm[GLAPP_CONFIG_PATCH_LEVEL_MAX].as<int>();
	...
	win.reset(
		new glapp::window(
			"tessViewer " TV_VERSION,
			setting.window_resolution_x,
			setting.window_resolution_y,
			4, 5,
			4,
			(int)setting.window_vsync,
			setting.window_full_screen ? glapp::window::init_flag::FULLSCREEN : glapp::window::init_flag::DEFAULT)
		);
	win->SetBackColor(glm::vec4(glm::vec3(0.5f), 1));

	if (GL_TRUE != glfwExtensionSupported("GL_ARB_direct_state_access"))
		throw std::runtime_error("OpenGL Unsupported : GL_ARB_direct_state_access");
	if (GL_TRUE != glfwExtensionSupported("GL_ARB_tessellation_shader"))
		throw std::runtime_error("OpenGL Unsupported : GL_ARB_tessellation_shader");
	...
	tv::model::default_glsl_info.GetUniformBuffer("MaterialOffset")->Update(&mat_offset);
	...
	draw_string->SetWindowSize(setting.window_resolution_x, setting.window_resolution_y);
	...
	for (picojson::value& o : objects) {
		models.emplace_back(o.get<picojson::object>(), /*shader_default,*/ material);
	}


	window_size = glm::ivec2(setting.window_resolution_x, setting.window_resolution_y);
	...
	cv::Mat flip_texture(cv::Size(default_texture.size[0], default_texture.size[1]), CV_8UC3);
	cv::flip(default_texture, flip_texture, 0);
	glPixelStorei(GL_UNPACK_ALIGNMENT, 4);
	glCreateTextures(GL_TEXTURE_2D, 1, &default_diffuse_texture);
	glTextureImage2DEXT(default_diffuse_texture, GL_TEXTURE_2D, 0, GL_COMPRESSED_RGB, flip_texture.cols, flip_texture.rows, 0, GL_BGR, GL_UNSIGNED_BYTE, flip_texture.data);
	glTextureParameteri(default_diffuse_texture, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTextureParameteri(default_diffuse_texture, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
...
void app::Run()
{
	...
	std::string patch_type(tv::model::default_patch_type == OpenSubdiv::Far::PatchDescriptor::Type::REGULAR ?
		"REGULAR" :
		"GREGORY_BASIS");
	...
	while (mainloop && !glfwWindowShouldClose(w)) {
		...
		draw_string->Set(5, 0,	(boost::format("FPS          %.1f") % (1.f / frametime)).str());
		...
		draw_string->Set(5, 72, (formater % "Patch Level" % tv::model::default_patch).str());
		draw_string->Set(5, 90, (formater % "Draw Call" % tv::model::draw_call).str());
		draw_string->Draw();
...
void app::osdErrorCallback(OpenSubdiv::Far::ErrorType err, const char * message)
{
...
void app::KeyDefaultCallback(GLFWwindow * window, int key, int scancode, int action, int mods)
{
	...
	if (action == GLFW_PRESS) {
		...
		case GLFW_KEY_LEFT_SHIFT:
			...
			break;
		case GLFW_KEY_UP:
			a->tess_fact = glm::min(a->tess_fact + 1, a->setting.graphics_osd_tess_level_max);
		...
		case GLFW_KEY_SPACE:
			std::cout 
				...
				<< (a->formater % "Angle" % glm::to_string(a->camera.Angle)).str()
				<< (a->formater % "LookPoint" % glm::to_string(a->camera.LookPoint)).str()
				<< (a->formater % "Right" % glm::to_string(a->camera.Right)).str()
...
void app::KeyFlyModeCallback(GLFWwindow * window, int key, int scancode, int action, int mods)
{
	app* a = (app*)glfwGetWindowUserPointer(window);
	...
	if (action == GLFW_PRESS) {
		...
		case GLFW_KEY_F:
			...
			glfwSetMouseButtonCallback(a->win->GetWin(), MouseButtonDfaultCallback);
...
void app::DragCameraRotate(GLFWwindow * window, double x, double y)
{
	...
	glm::vec2 move((float)x - a->previousMousePos.x, (float)y - a->previousMousePos.y);
	move *= a->frametime;
	if (move.x != 0.f) {
	...
	if (move.y != 0.f) {
		glm::vec3 o(a->camera.Pos - a->camera.LookPoint);
		a->camera.Pos = glm::rotate(a->camera.Pos - a->camera.LookPoint, move.y, -a->camera.Right) + a->camera.LookPoint;
		a->camera.Angle = glm::normalize(a->camera.LookPoint - a->camera.Pos);
		a->camera.Right = glm::cross(a->camera.Angle, glm::vec3(0.f, 1.f, 0.f));
		a->camera.Up = glm::cross(a->camera.Right, a->camera.Angle);
	}
...
void app::DragCameraTranslation(GLFWwindow * window, double x, double y)
{
	...
	glm::vec2 move((float)x - a->previousMousePos.x, (float)y - a->previousMousePos.y);
	move *= a->frametime * 0.1f * glm::length(a->camera.Pos - a->camera.LookPoint);
...
void app::DragCameraFlyMode(GLFWwindow * window, double x, double y)
{
	...
	if (move.y != 0.f) {
		a->camera.Angle = glm::rotate(a->camera.Angle, -move.y, a->camera.Right);
		a->camera.Right = glm::cross(a->camera.Angle, glm::vec3(0, 1, 0));
		a->camera.Up = glm::cross(a->camera.Right, a->camera.Angle);
...
void app::MouseButtonDfaultCallback(GLFWwindow * window, int button, int action, int mods)
{
	app* a = (app*)glfwGetWindowUserPointer(window);
	...
	if (action == GLFW_PRESS) {
		...
		case GLFW_MOUSE_BUTTON_MIDDLE:
			if (mods == GLFW_MOD_SHIFT) {
				glfwSetCursorPosCallback(a->win->GetWin(), DragCameraTranslation);
...
void app::MouseButtonFlayModeCallback(GLFWwindow * window, int button, int action, int mods)
{
	app* a = (app*)glfwGetWindowUserPointer(window);
...
void app::MouseScrollLengthCallback(GLFWwindow * window, double up, double down)
{
	app* a = (app*)glfwGetWindowUserPointer(window);
...
void app::WindowResizeCallback(GLFWwindow * window, int x, int y)
{
	...
	glViewport(0, 0, x, y);
	a->window_size.x = x; a->window_size.y = y;
	a->UpdateProjection();
...
void app::UpdateView()
{
	if (camera.fly_mode) {
		if (glm::length(camera.Move) > 0.1f) {
			glm::vec3 m = glm::normalize(camera.Angle * camera.Move.y + camera.Right * camera.Move.x) * frametime;
			camera.Pos += m;
			camera.LookPoint += m;
		}
	}
	transform.view = glm::lookAt(camera.Pos, camera.Pos + camera.Angle, camera.Up);// glm::vec3(0, 1, 0));
}
void app::UpdateProjection()
{
	transform.projection = glm::perspective(glm::radians(camera.Fov), (float)window_size.x / (float)window_size.y, 0.01f, 100.0f);
}
void app::UpdateUBO()
{
	tess.Update(tess_fact);
	tv::model::default_glsl_info.GetUniformBuffer("Transform")->Update(&transform);
	tv::model::default_glsl_info.GetUniformBuffer("LightInfo")->Update(&light);
	tv::model::default_glsl_info.GetUniformBuffer("Tessellation")->Update(&tess);
